...
export default function RetailerDashboard() {
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      try {
        ...
        const returnsRes = await axios.get(`http://localhost:5000/api/returnpackaging/all?status=${activeTab}`, {
          headers: { Authorization: `Bearer ${token}` },
        });
  ...
  const handleAction = async (id, action) => {
    const token = localStorage.getItem("token");
    try {
      await axios.put(`http://localhost:5000/api/returnpackaging/${action}/${id}`, {}, {
        headers: { Authorization: `Bearer ${token}` },
      });
  ...
  const membershipLevel =
    retailer.greenPoints >= 100 ? "🌍 Green Champion" :
    retailer.greenPoints >= 50 ? "🌱 Silver Partner" :
    "🌿 Bronze Partner";
    ...
    <div className="max-w-6xl mx-auto mt-24 px-4">
      ...
      <div className="flex flex-col sm:flex-row items-center justify-between mb-8 bg-white shadow p-6 rounded-lg">
        <div className="flex items-center gap-4">
          <img src={profileImg} alt="Profile" className="w-16 h-16 rounded-full border object-cover" />
          <div>
            <h1 className="text-2xl font-bold text-green-700">Welcome, {retailer.name}</h1>
            <p className="text-gray-500">Membership: <strong>{membershipLevel}</strong></p>
          </div>
        </div>
        <div className="mt-4 sm:mt-0 text-gray-700">
          <p><strong>Email:</strong> {retailer.email}</p>
          <p><strong>Mobile:</strong> {retailer.mobile || "Not Provided"}</p>
          <p><strong>Green Points:</strong> {retailer.greenPoints}</p>
        </div>
      </div>

      {/* Stats Cards */}
      <div className="grid sm:grid-cols-2 md:grid-cols-4 gap-6 mb-8">
        <Card title="Total Approved" desc={`${retailer.totalApproved || 0} returns approved`} />
        <Card title="Total Rejected" desc={`${retailer.totalRejected || 0} returns rejected`} />
        <Card title="Your Rank" desc={`#${retailer.rank || 'N/A'} based on approvals`} />
      ...
      <div className="bg-white shadow p-6 rounded-lg">
        <h2 className="text-lg font-semibold mb-4">
          {activeTab.charAt(0).toUpperCase() + activeTab.slice(1)} Packaging Returns
        </h2>
        ...
        {loading ? (
          ...
          <ul className="space-y-3">
            {returns.map((ret) => (
              <li key={ret.id} className="border p-4 rounded flex justify-between items-center flex-wrap">
                <div className="mb-2">
                  <p className="font-medium">User: {ret.user?.name}</p>
                  <p>Material: {ret.order?.product?.material}</p>
                  <p>Size: {ret.order?.product?.size}</p>
                  <p className="text-sm text-gray-500">Initiated At: {new Date(ret.createdAt).toLocaleString()}</p>
                </div>
                {activeTab === "pending" && (
                  <div className="flex gap-2">
                    <button onClick={() => openScanForm(ret)} className="bg-green-600 text-white px-3 py-1 rounded hover:bg-green-700">Approve</button>
                    <button onClick={() => handleAction(ret.id, "reject")} className="bg-red-600 text-white px-3 py-1 rounded hover:bg-red-700">Reject</button>
                  </div>
                )}
              </li>
            ))}
      ...
      {showScan && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
          <div className="bg-white p-6 rounded shadow-lg max-w-sm w-full">
            ...
            <select value={size} onChange={(e) => setSize(e.target.value)} className="border px-3 py-2 w-full mb-4">
